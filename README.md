...
| 食材のサジェスト | 在庫や食事の記録で、打った文字から食材を絞り込める |
| 献立以外の食事 | 提案外に食べたものも記録して摂取量に反映する |
| 部位の除外 | 怪我などで避けたい部位をメニューから外せる |
...
`npm run dev` / `npm run build` の前に、実行資産が自動で用意されます。
どちらもサイズが大きいので Git には含めていません。

- Tesseract の実行資産（約13MB）: `node_modules` から `public/tesseract/` へコピー
- ドットフォント DotGothic16（約0.7MB / 123サブセット）: `public/fonts/` へダウンロード
  （`next/font/google` が日本語サブセットに対応していないため自前で配信します。
  取得に失敗してもビルドは止まらず、システムのゴシック体で表示されます）
...
**見た目** — RPGのコマンドウィンドウ調（黒地に白の二重枠、ドットフォント、
`▶` カーソル）。`app/globals.css` のトークンと `components/ui.tsx` の
`Card` / `SectionTitle` / `Bar` / `Button` に寄せてあるので、
各画面の記述をほとんど変えずに見た目を差し替えられる構造になっています。
